...
class Add_Sign():
    ...
    def add_sign(self):
        ...
        action.click_and_hold(source1).move_by_offset(400, 150).pause(2).move_by_offset(-10, -10).release().perform()
        time.sleep(2)
        WebDriverWait(self.driver, 20).until(
            EC.element_to_be_clickable(('css selector', self.select_approver2))).click()
        source1 = self.driver.find_element('css selector', self.signature_field)
        action = ActionChains(driver)
        action.click_and_hold(source1).move_by_offset(900, 150).pause(2).move_by_offset(-10, -10).release().perform()
        time.sleep(2)
